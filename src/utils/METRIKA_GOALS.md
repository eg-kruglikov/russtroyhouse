...
### CTA на страницах ремонта

| Цель                     | Описание                                 | Компонент                 |
| ------------------------ | ---------------------------------------- | ------------------------- |
| `repair_consult_click`   | Клик по кнопке «Получить консультацию»   | RepairPage (динамическая) |
| `repair_portfolio_click` | Переход к проекту из блока «Наши работы» | RepairPage (динамическая) |
...
| Цель                | Описание                                           | Условия                         | Триггер            |
| ------------------- | -------------------------------------------------- | ------------------------------- | ------------------ |
| `engagement_soft`   | Мягкая вовлеченность                               | ≥1 мин, ≥1 страница, скролл 75% | useEngagementGoals |
| `engagement_medium` | Средняя вовлеченность                              | ≥2 мин, ≥2 страницы, скролл 75% | useEngagementGoals |
| `engagement_deep`   | Глубокая вовлеченность (теплая аудитория/retarget) | ≥3 мин, ≥3 страницы, скролл 75% | useEngagementGoals |

### Медиа и интерактив

| Цель             | Описание                                        | Компонент                        |
| ---------------- | ----------------------------------------------- | -------------------------------- |
| `video_sound_on` | Пользователь включил звук в полноэкранном видео | FullWidthViewportVideo           |
| `gallery_swipe`  | Пользователь пролистал галерею/свайпнул слайд   | FullWidthImageGallery, PhotoGrid |
